...
function LocalSource(conf) {
  ...
  this.handleDir = function(parentFolder, callBack) {
    ...
    for (idx in dirContent) {
      ...
      if (isDir == true) {
        ...
        if (parentFolder.thumb === undefined && (fileExtension == ".jpeg" || fileExtension == ".jpg")) {
          parentFolder.thumb = 'stream;' + absolutePath;
        }
    ...
    if (parentFolder.thumb !== undefined) {
      for (idx in parentFolder.audioFiles) {
        parentFolder.audioFiles[idx].thumb = parentFolder.thumb;
      }
    }
